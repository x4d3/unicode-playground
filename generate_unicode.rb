...
File.open(output_file, "w") do |file|
  ...
  # Iterate over each script range
  script_ranges.each do |script, range|
    # Write the script name as a header
    file.puts "=== #{script} ==="

    # Initialize line length counter
    line_length = 0

    # Write each character in the range to the file
    range.each do |codepoint|
      character = [codepoint].pack("U")
      # Check if adding the character would exceed the line length limit
      if line_length + character.length > 120
        # Start a new line
        file.puts
        line_length = 0
      end
      ...
      line_length += character.length
    end

    # Add a newline character after each script's characters
    file.puts
  end
end

puts "Unicode characters grouped by script property have been saved to #{output_file}."
